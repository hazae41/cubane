...
test("abi", async ({ message, test }) => {
  const bytes = tryEncode(Bool.new(true), String.new("hello"), Uint256.new(123n), Abi.DynamicBytes.new(Bytes.random(123))).unwrap()
  console.log(message, Bytes.toHex(bytes))
  const abi = tryDecode(bytes, Bool, String, Uint256, Abi.DynamicBytes).unwrap()
