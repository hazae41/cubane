...
delete untypedData.types.EIP712Domain
...
if (false) {
  ...
  const viemResult = benchSync("viem", () => {
    viem.hashTypedData(typedData as any)
  }, options)

  const ensResult = benchSync("ethers", () => {
    ethers.TypedDataEncoder.hash(untypedData.domain, untypedData.types, untypedData.message)
  }, options)
