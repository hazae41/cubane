...
export interface DynamicArray<T extends Factory, N extends number> extends Writable<never, Error> {
  readonly class: Factory<DynamicArray<T, N>>
  readonly inner: ReadOutputs<T[]> & { length: N }
  readonly count: N
}

export const createDynamicArray = <T extends Factory, N extends number>(factory: T, count: N) => class Class {
  ...
  static tryRead(cursor: Cursor): Result<DynamicArray<T, N>, DecodingError> {
    return Result.unthrowSync(t => {
